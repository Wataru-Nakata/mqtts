...
class TTSDecoder(nn.Module):
    def __init__(self, hp, phoneset_size):
        ...
        self.encoder = TransformerEncoder(
            nn.ModuleList([TransformerEncoderLayer(hp) for i in range(hp.enc_nlayers)])
        )
        self.decoder = TransformerDecoder(
            nn.ModuleList(
                [
                    TransformerDecoderLayer(hp, with_cross_attention=False)
                    for i in range(hp.dec_nlayers)
                ]
            )
        )
    ...
    def forward(self, q, phone, spkr, q_mask, phone_mask):
        ...
        ex_phone_mask = (
            torch.cat(
                [
                    torch.zeros(
                        (spkr.size(0), 1), device=spkr.device, dtype=torch.bool
                    ),
                    phone_mask,
                ],
                1,
        ...
        phone, enc_attn = self.encoder(
            phone, mask=None, attn_bias=phone_alibi, src_key_padding_mask=ex_phone_mask
        )
        phone = phone[:, 1:]
        # Run decoder
        q_mask = (
        ...
        audio_alibi[:, 0] = 0
        audio_alibi[:, :, 0] = 0
        output, _, dec_attn, _ = self.decoder(
            q,
            memory=None,
            tgt_mask=tgt_mask,
    ...
    def encode_phone(self, phone, spkr, phone_mask):
        phone = self.layer_norm_phone(phone)
        phone = torch.cat([spkr, phone], 1)
        ex_phone_mask = torch.cat(
            [
                torch.zeros((spkr.size(0), 1), device=spkr.device, dtype=torch.bool),
                phone_mask,
            ],
            1,
        )
    ...
    def inference_topkp_sampling_batch(
        self, phone, spkr, phone_mask, prior=None, output_alignment=False
    ):
        ...
        inps = inp
        # Decode
        past_kvs1, past_kv_cross, past_kvs2, clusters = (
            None,
            None,
            None,
            torch.empty(
                [batch_size, 0, self.hp.n_cluster_groups],
                device=phone.device,
                dtype=torch.long,
            ),
        )
        ...
        length_counter = torch.zeros(
            [batch_size], device=phone.device, dtype=torch.long
        )
        real_phone_lengths = (~phone_mask).long().sum(-1)  # N,
        if output_alignment:
            assert (
                batch_size == 1
            ), "Now only support output alignment for bs = 1 for debugging issues..."
            alignment = torch.zeros(
                (1, self.hp.max_output_length, self.hp.max_output_length),
                device=phone.device,
            )
        for i in range(self.hp.max_output_length):
            cond, _, _, new_1 = self.decoder(
                inp,
                memory=None,
                attn_bias=audio_alibi,
                tgt_mask=tgt_mask,
            ...
            selected_phone = selected_phone[index_map].reshape(
                batch_size, self.hp.phone_context_window, c_length
            )  # N*3, C
            # Mask for the starting phones
            phone_mask = torch.arange(
                self.hp.phone_context_window, device=phone.device
            ).repeat(batch_size, 1)
            phone_mask = phone_mask <= (back_map[:, -1:] + 1).expand(
                -1, self.hp.phone_context_window
            )
            phone_mask = ~phone_mask
            cond, _align = self.aligner(
                cond,
                selected_phone,
                tgt_mask=tgt_mask,
                memory_key_padding_mask=phone_mask,
            ...
            for j in range(self.hp.n_cluster_groups):
                ...
                if self.hp.use_repetition_gating:
                    logit[:, self.hp.n_codes + 2] = torch.min(
                        torch.max(logit[:, : self.hp.n_codes]),
                        logit[:, self.hp.n_codes + 2],
                    )
                # Top_p
                if self.hp.top_p < 1.0 and self.hp.top_p > 0.0:
                    ...
                    additional_prob = (
                        self.hp.length_penalty_max_prob - self.hp.top_p
                    ) * (length_counter / self.hp.length_penalty_max_length)
                    idx_to_remove = cum_probs > (
                        self.hp.top_p + additional_prob
                    ).unsqueeze(-1)
                ...
                if self.hp.use_repetition_token:
                    if clusters.size(1) == 0:  # First token, random choice
                        idx[idx == (self.hp.n_codes + 2)] = torch.randint(
                            self.hp.n_codes, size=(1,), device=idx.device
                        )
                    else:
                        idx[idx == (self.hp.n_codes + 2)] = clusters[:, -1:, j][
                            idx == (self.hp.n_codes + 2)
                        ]
                output.append(idx)
            output = torch.cat(output, 1).unsqueeze(1)  # N, 1, n_groups
            # Stop criterion
            stopping_streams = back_map[:, -1] == (
                real_phone_lengths - self.hp.phone_context_window
            )
            stopping_streams = (
                stopping_streams & self.transducer.is_end_token_batch(output)
            ) | (
                stopping_streams
                & (
                    torch.argmax(_align[:, 0, -1], dim=-1)
                    == self.hp.phone_context_window - 1
                )
            )  # N,
            ...
            # Update args
            tgt_mask = (
                self.tgt_mask[i + 3 + prior_size, : i + 3 + prior_size]
                .to(phone.device)
                .unsqueeze(0)
            )
            ...
            if i == 0:
                ...
                for ii, (p, n) in enumerate(
                    zip(past_kvs1, news[: self.hp.dec_nlayers])
                ):
