...
class Generator(torch.nn.Module):
    def __init__(self, h):
        ...
        self.spkr_linear = nn.Sequential(
            nn.Linear(512, 512), nn.LeakyReLU(LRELU_SLOPE), nn.Linear(512, 512)
        )
...
class Quantizer(torch.nn.Module):
    def __init__(self, h):
        ...
        self.quantizer_modules = nn.ModuleList(
            [
                Quantizer_module(h.n_codes, h.hidden_dim // h.n_code_groups)
    ...
    def forward(self, xin):
        ...
        x = xin.reshape(-1, self.h.hidden_dim)
        x = torch.split(x, self.h.hidden_dim // self.h.n_code_groups, dim=-1)
        min_indicies = []
        z_q = []
