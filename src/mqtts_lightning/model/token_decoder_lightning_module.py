...
class TokenDecoderLightningModule(LightningModule):
    def __init__(self, cfg: DictConfig, phoneset_size) -> None:
        ...
        self.cross_entropy = CrossEntropyLoss(
            label_smoothing=cfg.model.token_decoder.label_smoothing
        )
        self.phone_embedding = Embedding(
            phoneset_size, cfg.model.token_decoder.decoder.hidden_size, padding_idx=0
        )
        self.spkr_embedding = Embedding(
            cfg.model.token_decoder.n_speakers,
            cfg.model.token_decoder.decoder.hidden_size,
        )
        quantizer = QuantizerLightningModule.load_from_checkpoint(
            cfg.data.quantizer_path, cfg=cfg
        )
    ...
    def configure_optimizers(self):
        optimizer = hydra.utils.instantiate(
            self.cfg.model.token_decoder.optim, params=self.parameters()
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        recons_segments = self.token_decoder(
            batch["input_tokens"],
            phone_features,
            speaker_embedding,
            batch["tokens_mask"],
        ...
        targets = recons_segments["logits"][~batch["tokens_mask"]].view(
            -1, self.n_codes
        )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx < self.cfg.model.token_decoder.logging_samples:
            phone_mask = torch.full(
                (phone_features.size(0), phone_features.size(1)),
                False,
                dtype=torch.bool,
                device=phone_features.device,
            )
            synthetic, infer_attn = self.token_decoder.inference_topkp_sampling_batch(
                phone_features,
                speaker_embedding,
                phone_mask,
                prior=None,
            ...
            gt_speech = self.infer_vocoder(
                batch["input_tokens"][:, 1:], batch["speaker"]
            )
            self.log_audio(
                synthetic.cpu().view(-1),
                f"synthetic/{batch['filenames'][0]}",
                self.cfg.sample_rate,
            )
            self.log_audio(
                gt_speech.cpu().view(-1),
                f"gt/{batch['filenames'][0]}",
                self.cfg.sample_rate,
            )
