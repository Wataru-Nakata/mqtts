...
from mqtts_lightning.model.quantizer import (
    Encoder,
    Generator,
...
class QuantizerLightningModule(LightningModule):
    def __init__(self, cfg: DictConfig) -> None:
        ...
        if not self.use_xvector:
            self.speaker_embedding = nn.Embedding(
                cfg.model.quantizer.speaker_embedding.n_speakers,
                cfg.model.quantizer.speaker_embedding.embedding_dim,
            )
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.global_step >= self.cfg.model.quantizer.adversarial_start_step:
            ...
            # mpd
            mpd_out_real, mpd_out_fake, _, _ = self.mpd(wav, wav_generator_out.detach())
            loss_disc_f, losses_disc_f_r, losses_disc_f_g = discriminator_loss(
                mpd_out_real, mpd_out_fake
            )

            # msd
            msd_out_real, msd_out_fake, _, _ = self.msd(wav, wav_generator_out.detach())
        ...
        loss_g = (
            loss_recons * self.cfg.model.quantizer.loss.recons_coef
            + loss_q * self.cfg.model.quantizer.loss.quantizer_coef
        )
        if self.global_step > self.cfg.model.quantizer.adversarial_start_step:
            (
                mpd_out_real,
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss_recons = self.reconstruction_loss(
            wav_mel[:, :, :min_len], predicted_mel[:, :, :min_len]
        )
        if (
    ...
    def configure_optimizers(self):
        if self.use_xvector:
            opt_g = hydra.utils.instantiate(
                self.cfg.model.quantizer.optim.opt_g,
