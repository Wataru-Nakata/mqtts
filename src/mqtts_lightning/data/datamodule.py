...
class MQTTSDataModule(lightning.LightningDataModule):
    ...
    def collate_fn(self, batch, segment_size: int = -1):
        ...
        outputs["tokens"] = pad_sequence(
            [torch.stack(b["quantized_code.pth"], dim=0).T for b in batch],
            batch_first=True,
        )
        outputs["tokens_mask"] = self.length_to_mask(
            torch.tensor([len(b["quantized_code.pth"][0]) for b in batch]) + 1
        )
        outputs["phones"] = pad_sequence(
            [
                torch.tensor([self.vocab[p] for p in b["phones.txt"].split("|")])
                for b in batch
            ],
            batch_first=True,
        )
        outputs["phones_mask"] = self.length_to_mask(
            torch.tensor([len(b["phones.txt"].split("|")) for b in batch])
        )

        start = torch.ones((len(batch), 1, 4), dtype=torch.int) * (
            self.cfg.model.quantizer.quantizer.n_codes + 1
        )
        end = torch.ones((1, 4), dtype=torch.int) * (
            self.cfg.model.quantizer.quantizer.n_codes + 2
        )

        outputs["input_tokens"] = torch.cat([start, outputs["tokens"]], dim=1)
        end_added = [
            torch.cat([torch.stack(b["quantized_code.pth"]).T, end], dim=0)
            for b in batch
        ]
        ...
        outputs["speaker"] = torch.tensor(
            [self.speaker_dict[b["speaker.txt"]] for b in batch]
        )
        return outputs
